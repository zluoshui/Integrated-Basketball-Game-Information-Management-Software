...
internal static class SelfCheck
{
    public static void Run()
    {
        ...
        data.Events.Add(new MatchEvent { MatchId = match.Id, PlayerId = awayPlayer.Id, Side = TeamSide.Away, Kind = MatchEventKind.Turnover });
        ...
        Assert(projection.PlayerStats.Any(item => item.PlayerName == awayPlayer.DisplayName && item.Steals == 1 && item.Turnovers == 1), "away player stats projection failed");

        data.Events.RemoveAt(data.Events.Count - 1);
        projection = Statistics.Compute(data, match);
        Assert(projection.PlayerStats.Any(item => item.PlayerName == awayPlayer.DisplayName && item.Turnovers == 0), "undo projection failed");
        ...
        try
        {
            ...
            Assert(loaded.Events.Count == 8, "data store event round-trip failed");
